...
function draw(ctx, s, selected, lastTap) {
  ...
  ctx.strokeRect(4, H / 2 + 4, W - 8, H / 2 - 8);
  ctx.setLineDash([]);
  ctx.fillStyle = "rgba(34, 211, 238, 0.95)";
  ctx.fillText("SEN — alt yariya bas", 8, H - 10);
  ctx.font = "bold 11px sans-serif";
  ctx.fillText("YERLESTIR", W / 2 - 34, H / 2 + 48);
  ...
  if (lastTap) {
    const [tx, ty] = map(lastTap.x, lastTap.y);
    ctx.strokeStyle = lastTap.own ? "#22d3ee" : "#f87171";
    ctx.lineWidth = 2;
    ctx.beginPath();
...
function pointerToSim(el, e) {
  ...
  if (y >= OWN_MAX_Y && y < OWN_MAX_Y + 0.12) y = OWN_MAX_Y - 0.02;
  return { x, y, py, own: y < OWN_MAX_Y };
}
...
export default function ArenaView() {
  ...
  useEffect(() => {
    ...
    const loop = (now) => {
      ...
      if (ctx && s) draw(ctx, s, selectedRef.current, lastTapRef.current);
  ...
  const tryPlace = (x, y, src, own = y < OWN_MAX_Y) => {
    lastTapRef.current = { x, y, own };
  ...
  const onCanvas = (e) => {
    ...
    if (!el) return;
    const pt = pointerToSim(el, e);
    tryPlace(pt.x, pt.y, pointerSrc(e), pt.own);
  };
  ...
  const onSeasonReset = () => {
    ...
    cupsRef.current = next;
    setCups(next);
    setQueue(createQueue({ cups: next }));
    const a0 = arenaOf(before);
    const a = arenaOf(next);
    setHint(`Sezon reset ${before}→${next} · ${a0.name}→${a.name} (#${a.id}) · kuyruk kupaya baglandi`);
  };
  ...
  return (
    <div style={{ background: "#020617", color: "#e2e8f0", minHeight: "100vh", padding: 12, fontFamily: "sans-serif" }}>
      ...
      <div style={{ fontSize: 12, marginBottom: 6, color: "#67e8f9" }}>
        Lig {lig.name} (#{lig.id}) · kupa {cups} · sezon sonrasi {seasonSoftReset(cups)} {arenaOf(seasonSoftReset(cups)).name}
      </div>
        ...
        {hand.map((id, i) => {
          ...
          return (
            <button key={i} onClick={() => setSelected(id)} style={{
              ...
              <div style={{ fontSize: 11, opacity: 0.9 }}>E {cost} · HP {UNITS[id].hull}</div>
            </button>
          );
        })}
      </div>
      <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
        ...
        <button onClick={onSeasonReset} style={{ padding: "8px 12px", background: "#0e7490", color: "#ecfeff", border: 0, borderRadius: 8 }}>sezon reset → {seasonSoftReset(cups)}</button>
        ...
        <div style={{ marginTop: 12, fontSize: 12 }}>
          ...
          {receipt && (
            <div style={{ marginTop: 6, color: "#94a3b8" }}>
              ...
              {" "}kupa {receipt.trophies.before[0]}→{receipt.trophies.after[0]}
              {" "}\u00b7 {arenaOf(receipt.trophies.before[0]).name}\u2192{arenaOf(receipt.trophies.after[0]).name}
              {" "}(#{receipt.trophies.arena[0]})
              {stamp ? (
                <div>
